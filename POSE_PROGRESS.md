...
- 2026-05-16 retained landmarker `032->035->036` fused `DEPTHWISE->CONV2D->ADD` tile path:
  - Motivation:
    - Fresh-context review recommended moving away from small worker-count gates and fusing a real hot graph chain. It specifically pointed at landmarker `032 DEPTHWISE -> 035 CONV2D -> 036 ADD`, whose old path wrote and reread a `[1,64,64,96]` depthwise intermediate and used a second worker-pool dispatch for the packed pointwise/add.
    - The retained implementation is narrowly guarded to `pose_landmarker` source op `032` only. Each work item owns one `POSE_PW_TILE` pixel tile, computes the 3x3 SAME depthwise output into aligned stack scratch once, then loops all packed pointwise output-channel blocks and reuses the existing packed 1x1/add tile path. This avoids recomputing depthwise per output-channel block.
    - Added `POSE_FUSE_DW_PW_ADD=0` as an escape hatch for same-binary checks.
  - Build and local correctness:
    - Local checks passed:
      - `cc -O3 -std=c11 -Wall -Wextra -I out/pose_runtime_data scripts/pose_neon_runtime.c -o /tmp/pose_neon_runtime_dw_pw_local -lm -pthread`
      - `git diff --check`
      - `scripts/build_pose_runtime_aarch64.sh out/pose_neon_runtime_aarch64_dwpw32`
    - Local x86 landmarker exported outputs matched exactly with the fusion enabled vs disabled for tensors `283`, `310`, `312`, and `315`.
  - Baseline refreshed before editing on Pi 3B+ (`vcgencmd get_throttled` still reported `0x50005`):
    - `sha256sum out/pose_neon_runtime_aarch64_ofast`: `269f8e167e1b060b51ee89aae1ef1dbc08274abf18613e76fb174743245cea8e`.
    - Landmarker `058`/`061` short baseline, `bench-op`, `reps=40`, `warmup=5`:
      - `058 DEPTHWISE`: 2 workers `6.041 ms`, 3 workers `4.352 ms`, 4 workers `5.029 ms`.
      - `061 CONV2D+ADD`: 2 workers `1.802 ms`, 3 workers `1.069 ms`, 4 workers `0.957 ms`.
    - Tracked baseline on `out/human-for-pose.rgb`, `514x994`, `reps=32`, `refresh_interval=0`:
      - 2 workers `148.233 ms`, 3 workers `122.311 ms`, 4 workers `128.445 ms`.
  - Pi raw correctness for the fused candidate:
    - Detector 4-thread single rep: `299.573 ms`; tensor 441 max abs `5.264e-4`, tensor 429 `3.357e-4`.
    - Landmarker 4-thread single rep: `119.898 ms`; tensor 310 max abs `8.278e-3`, tensor 315 `2.86e-14`, tensor 283 `5.188e-4`, tensor 312 `1.359e-5`.
  - Targeted chain benchmark, old default depthwise plus old default pointwise/add sum vs fused candidate, `bench-op`, `reps=50`, `warmup=6`:
    - 2 workers: old `032` `8.780 ms` + old `035->036` `2.707 ms` = `11.487 ms`; fused `032->035->036` `9.490 ms`.
    - 3 workers: old `6.723 ms` + `2.376 ms` = `9.099 ms`; fused `7.378 ms`.
    - 4 workers: old `5.490 ms` + `1.972 ms` = `7.462 ms`; fused `6.356 ms`.
  - Integrated tracked A/B on `out/human-for-pose.rgb`, `514x994`, old default vs fused candidate:
    - First smoke, `reps=40`:
      - 2 workers: `147.506 -> 146.450 ms`.
      - 3 workers: `122.578 -> 120.168 ms`.
      - 4 workers: `123.114 -> 117.823 ms`.
      - Exported tracked JSON matched exactly in all modes: `0.0` max abs.
    - Repeated strict run, `reps=48`, old first:
      - Repeat 2: 2 workers `147.474 -> 146.299 ms`; 3 workers `121.279 -> 119.893 ms`; 4 workers `119.310 -> 116.828 ms`.
      - Repeat 3: 2 workers `147.887 -> 148.243 ms`; 3 workers `121.891 -> 119.002 ms`; 4 workers `118.936 -> 116.667 ms`.
      - Exported tracked JSON matched exactly in every mode and repeat: `0.0` max abs.
    - Follow-up 2-worker check, `reps=72`, candidate first to check ordering/noise:
      - Repeat 4: candidate `145.515 ms`, old `147.450 ms`.
      - Repeat 5: candidate `145.643 ms`, old `146.638 ms`.
      - Exported tracked JSON matched exactly: `0.0` max abs.
  - Final default binary refresh:
    - Rebuilt `out/pose_neon_runtime_aarch64_ofast` with the retained source and copied it to the Pi.
    - Pi sha256 for refreshed `ofast`: `d0ab0d7e55766b9f0aeb799267ada772d910ae8e0dee9c955bf3dd6f60cb7750`.
    - Final 4-worker landmarker raw check: `118.429 ms`; tensor diffs stayed in the usual range.
    - Final 4-worker tracked smoke, `reps=48`, `refresh_interval=0`: tracked frame `117.628 ms`, `8.501 FPS`, amortized `123.577 ms`, `8.092 FPS`.
  - Decision:
...
- 2026-05-16 retained landmarker `046->049->052` fused `PAD->DEPTHWISE->CONV2D` tile path:
  - Motivation:
    - This is the follow-up candidate identified after rejecting the `058->061->062` scratch-only fusion. Landmarker `046 PAD` feeds only `049 DEPTHWISE`, and `049` feeds only `052 CONV2D`, so the padded `[1,68,68,96]` tensor and the depthwise `[1,32,32,96]` tensor can be skipped for this fixed graph section.
    - The target shape is exact: pad top/left/bottom/right by 2, then 5x5 stride-2 depthwise with ReLU6 over `96` channels, then packed 1x1 pointwise to `24` channels.
  - Implementation:
    - Added a narrow `can_fuse_pad_depthwise_pointwise` guard for `pose_landmarker` ops `046/049/052` only, with `POSE_FUSE_PAD_DW_PW=0` as an escape hatch.
    - Added a fused tile worker that computes a `POSE_PW_TILE x 96` depthwise scratch tile directly from the original unpadded input, then immediately runs the existing packed 1x1 tile path for all output-channel blocks.
    - Added an AArch64 fast path for six same-row interior stride-2 output pixels using `fmaq8_depthwise_5x5s2_six_row`; border and short-tail tiles fall back to checked per-pixel code.
    - Added `bench-op` support for the fused triplet so requests for op `46`, `49`, or `52` can time the fused chain.
  - Build and local correctness:
    - Passed `git diff --check`.
    - Local build passed:
      - `cc -O3 -std=c11 -Wall -Wextra -I out/pose_runtime_data scripts/pose_neon_runtime.c -o /tmp/pose_neon_runtime_pad49_local -lm -pthread`
    - Cross-build passed:
      - `scripts/build_pose_runtime_aarch64.sh out/pose_neon_runtime_aarch64_ofast`
  ...
  - Pi raw correctness and isolated fused-op timing:
    - Candidate binary before final install: `out/pose_neon_runtime_aarch64_pad_dwpw49`, sha256 `2b38c237a1a16c9d12736fc7b9304344d363852e151bac3dd879e3bb63dd6a16`.
    - Detector 4-thread single rep: `293.161 ms`; tensor 441 max abs `5.264e-4`, tensor 429 `3.357e-4`.
    - Landmarker 4-thread single rep: `119.760 ms`; tensor 310 max abs `8.278e-3`, tensor 315 `2.86e-14`, tensor 283 `5.188e-4`, tensor 312 `1.359e-5`.
    - Targeted same-binary `bench-op 49`, old separated path forced with `POSE_FUSE_PAD_DW_PW=0`, `reps=50`, `warmup=6`:
      - 2 workers: old `049` `9.503 ms` + old `052` `0.896 ms` = `10.398 ms`; fused `8.727 ms`.
      - 3 workers: old `7.424 ms` + `0.641 ms` = `8.064 ms`; fused `6.369 ms`.
      - 4 workers: old `7.406 ms` + `0.576 ms` = `7.982 ms`; fused `6.495 ms`.
  - Integrated tracked A/B on `out/human-for-pose.rgb`, `514x994`, `reps=48`, `refresh_interval=0`, current retained `ofast` vs candidate:
    - Initial continuous run started under `vcgencmd get_throttled=0x50005`.
      - Repeat 1: 2 workers `146.057 -> 144.919 ms`; 3 workers `119.998 -> 119.257 ms`; 4 workers `116.564 -> 115.606 ms`.
      - Repeat 2 completed for 2 and 3 workers: 2 workers `145.663 -> 144.913 ms`; 3 workers `120.462 -> 118.312 ms`.
      - The Pi rebooted during repeat 2's old 4-worker run. After reboot, `uptime` was about one minute and `dmesg` reported `Undervoltage detected!`, so that interrupted benchmark was discarded as a board/power failure, not a candidate runtime result.
    - Rerun in shorter batches:
      - Batch A: 2 workers `147.809 -> 146.344 ms`; 3 workers `121.757 -> 120.543 ms`; 4 workers `119.334 -> 121.788 ms` (4-worker regression).
      - Batch B, 4 workers: `117.641 -> 116.305 ms`.
      - Batch C, reversed order, 4 workers, `reps=72`: candidate `116.632 ms`; old second `117.508 ms`.
    - Exported tracked JSON matched exactly in every completed batch and worker mode: `0.0` max abs over `355` numeric fields.
  - Final default binary refresh:
    - Rebuilt `out/pose_neon_runtime_aarch64_ofast`, copied it to the Pi, and installed it over the retained binary.
    - Pi sha256 for refreshed `ofast`: `9dace9b3adbfd0aca769f399e92def456434f68effb44ba67bf934661c79ff3e`.
    - Final raw tensor smoke with default binary:
      - Detector 4-thread single rep: `257.774 ms`; tensor 441 max abs `5.264e-4`, tensor 429 `3.357e-4`.
      - Landmarker 4-thread single rep: `148.933 ms`; tensor 310 max abs `8.278e-3`, tensor 315 `2.86e-14`, tensor 283 `5.188e-4`, tensor 312 `1.359e-5`.
    - Final 4-worker tracked smoke, `reps=48`, `refresh_interval=0`: tracked frame `117.137 ms`, `8.537 FPS`; amortized `123.478 ms`, `8.099 FPS`.
  - Decision:
    - Retained. The isolated fused chain is a clear 2/3/4-worker win, the tracked path improves in all completed 2- and 3-worker A/B runs, and 4-worker improves in the initial run plus two follow-up checks including reversed-order `reps=72`.
    - The single batch-A 4-worker regression is recorded as residual Pi noise around an under-voltage/reboot window. Future changes should keep using repeated tracked A/B and should not stack on this pattern unless they beat the tracked path, not only the fused op.

- 2026-05-16 retained landmarker `006->009` fused `DEPTHWISE->CONV2D` tile path:
  - Motivation:
    - Fresh profile after `bc685c2` showed the tracked path at `115.442 ms` / `8.662 FPS` for a 24-frame 4-worker smoke, with landmarker totals still dominated by CONV2D `71.178 ms` and DEPTHWISE `69.808 ms`.
    - Top landmarker ops included retained fused `032` `6.878 ms`, retained fused `049` `6.567 ms`, then `006 DEPTHWISE` `6.240 ms`, first conv `003` `5.935 ms`, `058 DEPTHWISE` `5.403 ms`, and `012 CONV2D` `5.272 ms`.
    - A fresh-context reviewer independently ranked `006 DEPTHWISE -> 009 CONV2D` first: `[1,128,128,24]` 3x3 stride-1 SAME depthwise with ReLU6 followed by 1x1 pointwise to `[1,128,128,8]`. This is not one of the previously rejected no-op or worker-count gates.
  - Implementation:
    - Added a narrowly guarded `can_fuse_depthwise_pointwise` for `pose_landmarker` source ops `006/009` only, with `POSE_FUSE_DW_PW=0` as a same-binary disable switch.
    - Added a no-residual `op_depthwise_pointwise` tile worker. It computes a `POSE_PW_TILE x 24` depthwise scratch tile using the existing six-pixel `depthwise_3x3s1_same_tile_to_tmp`, then immediately calls the packed 1x1 tile path and skips materializing tensor `189`.
    - Added `bench-op` support for `DEPTHWISE+CONV2D`, so requests for op `006` or `009` can time the fused pair.
  - Build and local correctness:
    - Passed `git diff --check`.
    - Local build passed:
      - `cc -O3 -std=c11 -Wall -Wextra -I out/pose_runtime_data scripts/pose_neon_runtime.c -o /tmp/pose_neon_runtime_dw_pw6_local -lm -pthread`
    - Local landmarker outputs with the fusion enabled and disabled matched byte-for-byte for tensors `283`, `310`, `312`, and `315`.
    - Cross-builds passed:
      - `scripts/build_pose_runtime_aarch64.sh out/pose_neon_runtime_aarch64_dwpw6`
      - `scripts/build_pose_runtime_aarch64.sh out/pose_neon_runtime_aarch64_ofast`
  - Pi raw correctness and isolated fused-op timing:
    - Candidate binary sha256 before final install: `aab29dfd47af94898b06aaed8e65c55b6a06a5750bf9994035a485ed3d8a6113`.
    - Raw checks stayed in the usual range:
      - Detector 4-thread single rep: `305.187 ms`; tensor 441 max abs `5.264e-4`, tensor 429 `3.357e-4`.
      - Landmarker 4-thread single rep: `112.453 ms`; tensor 310 max abs `8.278e-3`, tensor 315 `2.86e-14`, tensor 283 `5.188e-4`, tensor 312 `1.359e-5`.
    - Targeted `bench-op`, old separated path forced with `POSE_FUSE_DW_PW=0`, `reps=50`, `warmup=6`:
      - 2 workers: old `006` `5.846 ms` + old `009` `2.106 ms` = `7.951 ms`; fused `006->009` `7.144 ms`.
      - 3 workers: old `4.234 ms` + `1.831 ms` = `6.065 ms`; fused `5.196 ms`.
      - 4 workers: old `3.939 ms` + `1.924 ms` = `5.864 ms`; fused `3.982 ms`.
  - Integrated tracked A/B on `out/human-for-pose.rgb`, `514x994`, `reps=48`, `refresh_interval=0`, current retained `ofast` vs candidate:
    - Batch A, old first:
      - 2 workers: `144.769 -> 142.295 ms`, `6.908 -> 7.028 FPS`.
      - 3 workers: `119.572 -> 116.548 ms`, `8.363 -> 8.580 FPS`.
      - 4 workers: `116.026 -> 113.457 ms`, `8.619 -> 8.814 FPS`.
    - Batch B, candidate first:
      - 2 workers: candidate `143.590 ms`, old second `145.684 ms`.
      - 3 workers: candidate `117.149 ms`, old second `119.661 ms`.
      - 4 workers: candidate `114.594 ms`, old second `116.515 ms`.
    - Exported tracked JSON matched exactly in every batch and worker mode: `0.0` max abs over `355` numeric fields.
  - Final default binary refresh:
    - Rebuilt and copied `out/pose_neon_runtime_aarch64_ofast` to the Pi. A first combined copy-plus-48-frame-smoke run rebooted the Pi during the final tracked smoke; `dmesg` again reported `Undervoltage detected!`, and after reboot the binary had rolled back to the previous sha. Re-copied the binary in a separate step and ran `sync` before benchmarking again.
    - Pi sha256 for refreshed `ofast`: `846246ef9d57ab7ac745f788b1161912332026cd1b75f3830662ac4e398eead6`.
    - Final raw landmarker check after the synced copy: `145.706 ms`; tensor diffs stayed in the usual range.
    - Final 4-worker tracked smoke after the synced copy, `reps=24`, `refresh_interval=0`: tracked frame `117.008 ms`, `8.546 FPS`; amortized `132.688 ms`, `7.536 FPS`.
  - Decision:

...
APP_NAME = "gemma4-omni-audio-conditioned-style-head-smoke"
...
def train_audio_conditioned_style_head(
        ...
        train_rows.append(
            {
                **{k: v for k, v in source_row.items() if k not in {"target_wav_bytes"}},
                "conditioning_text": make_audio_conditioning_text(source_row, include_style_labels),
    ...
    cond_vectors = []
    audio_input_records = []
    input_dir = Path("/tmp/gemma_audio_condition_inputs")
    input_dir.mkdir(parents=True, exist_ok=True)
    for row in train_rows:
        input_path = input_dir / f"{safe_filename(row['example_id'])}_input.wav"
        input_path.write_bytes(row["input_wav_bytes"])
        messages = [
            {
                "role": "user",
                "content": [
                    {"type": "audio", "audio": str(input_path)},
                    {"type": "text", "text": row["conditioning_text"]},
                ],
            }
        ]
        ...
        audio_input_records.append(
            {
                ...
                "input_audio_s": len(wav_bytes_to_array(row["input_wav_bytes"])) / SAMPLE_RATE,
    ...
    artifact_dir = Path("/tmp/gemma4_omni_audio_conditioned_style_head")
    ...
    torch.save(
        ...
        artifact_dir / "audio_conditioned_style_head.pt",
    )
    ...
    with zipfile.ZipFile(zip_buffer, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.write(artifact_dir / "audio_conditioned_style_head.pt", "audio_conditioned_style_head.pt")
        for row in train_rows:
            zf.writestr(f"input_audio/{row['example_id']}.wav", row["input_wav_bytes"])
        ...
        zf.writestr(
            "gemma4_omni_audio_conditioned_style_head_manifest.json",
            json.dumps(
                {
                    "gemma_model_id": GEMMA_MODEL_ID,
                    "snac_model_id": SNAC_MODEL_ID,
                    "sample_rate": SAMPLE_RATE,
                    "conditioning": "Gemma native audio frontend hidden state over input WAV plus compact fitness state",
                    "include_style_labels": include_style_labels,
                    "streaming_config": {
                        "first_level0_tokens": STREAM_FIRST_LEVEL0_TOKENS,
                        "next_level0_tokens": STREAM_NEXT_LEVEL0_TOKENS,
...
def main(
    ...
    out_dir: str = "out/gemma4_omni_audio_conditioned_style_head",
    max_steps: int = 200,
    limit: int = 0,
    include_style_labels: bool = False,
):
    ...
    result = train_audio_conditioned_style_head.remote(
    ...
    zip_path = target / "gemma4_omni_audio_conditioned_style_head_artifact.zip"
